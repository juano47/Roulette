//ruleta v10  agrega COLUMNAS / COLUMNAS INVERSO / COLORES (modificacion) / COLORES INVERSO
...
int main() {
	...
	do{
		do{	
			...
			cin.clear ();
			fflush(stdin);
			if(entrar<1||entrar>20) {
				system("cls");
				cout<<"\t\t\aError: la seleccion solo puede ser 1 a 20. Reingrese por favor"<<endl;
			}
			
		} while(entrar<1||entrar>20);
		...
		case 19: //ANALISIS AUTOMATICO
			...
			system("cls");
			break;
		case 20: if (Salir()){ // si deseaSalir es verdadera, entonces no se debe continuar con la ejecucion del programa.
			continuarMenu = false;
		}
			break;
		}
