//ruleta v11.1  ESPECIAL APUESTA DESPUES QUE SALEN 5 VECES SEGUIDAS UN COLOR - PAR/IMPAR - MAYOR/MENOR - ETC 
...
void colores(int vector[], int tl){
	...
	while(tl>0){
		...
		if(contador!=0){
			if(contador==7 && vectorAux[contador]!=0){ //controla que salio 7 veces el rojo y a continuacion no salio cero
				cout<<" "<<contador<<"	\t\t\t\t+1"<<endl; //agrega +1 si salio 7 veces el rojo y suma una ganancia
				ganancia++;
			}
			else if(contador>7){
				cout<<" "<<contador<<"	\t\t\t\t-1"<<endl; //agrega -1 si salio mas de 7 veces el rojo y resta una ganancia
				ganancia--;
			}
			else
				cout<<" "<<contador<<endl;
		...
		//si el contador=0 significa que se encontro con NEGRO o CERO
			...
			//si el contador=0 se encontro con NEGRO
				...
				if(contador!=0){
					if(contador==7 && vectorAux[contador]!=0){
